<?php
  ...
  if ( $odkaz_rewrite AND preg_match($vyraz, $url) ) {
    $url = preg_replace($vyraz, '\\1', $url); //odstraneni adr localhostu
  }
  
  $vyraz = '~(.*)\?(.*)~i';
  if ( preg_match($vyraz, $url) ) {
    $url = preg_replace($vyraz, '\\1', $url); //odstraneni vsech parametru za ? vcetne (Pristupne pres $_GET)
  }
  
  $vyraz = '~^/(.*)/$~i';
  $url = preg_replace($vyraz, '\\1', $url); //odstraneni prvniho a posledniho lomitka
  
  $request_url = explode("/", $url);
  $count = count($request_url);

// Spolecna data pro vsechny skripty
